...
export const TripApproval = ({ route, navigation }) => {
  ...
  // Use the trip and notification contexts
  const { updateTripStatus } = useTrips();
  const { loadNotifications } = useNotifications();

  useEffect(() => {
    console.log('TripApproval mounted with trip:', trip._id);
    console.log('Navigated from:', route.params?.from || 'unknown');
  }, []);
  ...
  useEffect(() => {
    const checkAdminStatus = async () => {
      try {
        ...
        setUserIsAdmin(adminStatus === 'true');
      } catch (error) {
  ...
  const handleApprove = async () => {
    try {
      ...
      const response = await updateTripStatus(trip._id, 'approved');
  ...
  const handleReject = async () => {
    try {
      ...
      // Use updateTripStatus from context
      const response = await updateTripStatus(trip._id, 'rejected');
