...
const AuthLoadingScreen = ({ navigation }) => {
  useEffect(() => {
    const checkAuthStatus = async () => {
      ...
      try {
        // Check if there's a force logout flag
        const forceLogout = await AsyncStorage.getItem('forceLogout');
        if (forceLogout === 'true') {
          console.log('Force logout flag detected, clearing all tokens and redirecting to Login');
          await AsyncStorage.multiRemove([
            ...
            'userName',
            'userEmail',
            'forceLogout' // Also remove the flag itself
          ]);
